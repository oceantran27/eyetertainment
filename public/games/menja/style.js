...
document.addEventListener("DOMContentLoaded", function () {
  setGameMode(GAME_MODE_RANKED); // Set the game mode to Normal
  setActiveMenu(null); // Hide any active menus
...
function endGame() {
  bgrSound.pause();
  playAndDisposeAudio("./audio/over.mp3");
  ...
  countdownElement.textContent = seconds.toString() + "s";
...
function tick(width, height, simTime, simSpeed, lag) {
  ...
  if (slowmoRemaining > 0) {
    ...
    bgrSound.playbackRate = 0.5;
    targetSpeed = pointerIsDown ? 0.075 : 0.3;
  } else {
    bgrSound.playbackRate = 1;
      ...
      if (distance <= targetHitRadius) {
        ...
        if (!target.hit) {
          ...
          if (pointerSpeedScaled > minPointerSpeed) {
            ...
            if (target.health <= 0) {
              playAndDisposeAudio("./audio/break.mp3");
            ...
            playAndDisposeAudio("./audio/hit.mp3");
            incrementScore(5);
